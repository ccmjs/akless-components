...
//    "user": [ "ccm.instance", "https://ccmjs.github.io/akless-components/user/versions/ccm.user-9.7.0.js" ]
    ...
    Instance: function () {
      ...
      let $, analytics, chat, kanban_board, main_elem, menu, team_data, team_nr, teambuild;
      ...
      this.start = async () => {
        ...
        // prepare team project analytics
        analytics = await this.analytics.start( { parent: null, project: [ 'ccm.instance', this.component.index, JSON.parse( this.config ) ] } );
      ...
      const renderMenu = async () => {
        menu = await this.menu.start( {
          ...
          data: { entries: this.entries.map( ( title, i, entries ) => { return { title: title, disabled: !( i === 0 || team_nr || i === entries.length - 1 ) } } ) },
          selected: 1,
          onchange: async event => {
            switch ( event.id ) {
              ...
              case 4: $.setContent( main_elem.querySelector( '#content' ),    analytics.root ); break;
            }
          }
        } );
      };
