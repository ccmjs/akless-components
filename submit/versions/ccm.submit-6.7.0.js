    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        async function handleSeveral( several ) {
          ...
          const name = template.querySelector( '[name]' ).getAttribute( 'name' );  // name attribute
        ...
        async function handleSpecial( input ) {
          ...
          switch ( type ) {
            case 'hidden':
              // input type hidden with no value? => default value is a unique string
              if ( !input.value ) input.value = $.generateKey();
            case 'button':
            case 'checkbox':
            case 'color':
            case 'date':
