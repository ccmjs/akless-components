 ...
 * @version 2.11.0
 * @changes
 * version 2.11.0 (10.04.2020):
 * - uses ccm v25.4.0
 * - renamed HTML class for fade effect from 'fade' to 'fading' to avoid conflicts with bootstrap
 ...
 * - added 'prev' method to select previous menu entry
 * - added 'next' method to select next menu entry
 * - added optional keyboard and remote control support
 * - added optional touch control support
 * version 2.9.0 (22.12.2019):
 * - added 'noclick' parameter for menu.select() to prevent click action
 * - uses ccm v24.2.0
 * version 2.8.5 (27.11.2019):
 * - uses ccm v24.1.1
 * version 2.8.4 (10.10.2019):
 * - uses ccm v24.0.1
...
//    "trigger_selected": true
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        function renderMenuEntry( entry_data, i ) {
          ...
          async function onClick() {
            ...
            // remember currently selected menu entry
            selected = entry_data.id;
             ...
             * @type {boolean}
             */
            const is_selected = entry_elem.classList.contains( 'selected' );
            ...
            if ( !is_selected || !self.deselectable ) {
            ...
            if ( is_selected && !self.trigger_selected ) return;

            // log 'click' event and trigger 'onchange' callback
            self.logger && self.logger.log( 'click', $.clone( event_data ) );
            self.onchange && self.onchange( Object.assign( event_data, { instance: self } ) );
            ...
            function performActions() {
              ...
              // prepare event data
              event_data = Object.assign( event_data, entry_data );
        ...
        /** when display is touched by user */
        ...
        /** when user moves his finger on display */
        function handleTouchMove( evt ) {
          if ( !x_start ) return;
          self[ x_start - evt.touches[ 0 ].clientX > 0 ? 'prev' : 'next' ]();
          x_start = null;
        }
      ...
      /**
       * returns the menu entry ID of the currently selected menu entry
       ...
       * @param {boolean} noclick - select active menu entry without trigger of click event
       */
      this.select = ( entry, noclick ) => {

        /**
         * menu entry to be selected
         * @type {Element}
         */
        const element = ( typeof entry === 'string' ? this.element.querySelector( '#entry-' + entry ) : this.element.querySelectorAll( '.entry' )[ entry - 1 ] );

        // already selected and should not be trigger again? => abort
        if ( element.classList.contains( 'active' ) && !this.trigger_selected ) return;
      ...
      this.prev = () => ( this.element.querySelector( '.entry.active' ).previousElementSibling || this.element.querySelector( '#entries' ).lastElementChild ).click();

      /** selects next menu entry */
      this.next = () => ( this.element.querySelector( '.entry.active' ).nextElementSibling || this.element.querySelector( '#entries' ).firstElementChild ).click();
