...
//    "routing": [ "ccm.instance", "https://ccmjs.github.io/akless-components/routing/versions/ccm.routing-2.0.3.js", { "app": true } ],ååå
//    "selected": 1,
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        function renderMenuEntry( entry_data, i ) {
           ...
           * @type {Element}
           */
          const entry_elem = $.html( self.html.entry, { id: entry_data.id, click: onClick } );
          ...
          async function onClick() {
            ...
            // update route
            self.routing && self.routing.set( entry_data.id );
      ...
      this.select = entry => {
        ...
        // select menu entry
        entry && ( typeof entry === 'string' ? this.element.querySelector( '#' + entry ) : this.element.querySelectorAll( '.entry' )[ entry - 1 ] ).click();
