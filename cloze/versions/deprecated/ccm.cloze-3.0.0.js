  ...
  //  onfinish: { "clear": true, "log": true },
    ...
    Instance: function () {
      ...
      this.start = callback => {
        ...
        if ( my.start_button ) $.setContent( self.element, $.html( my.html.start, start ) );
        ...
        function start() {
          ...
          function renderKeywords() {
            ...
            if ( my.keywords ) ( Array.isArray( my.keywords ) ? my.keywords : keywords ).map( keyword => {
            ...
            else return $.removeElement( keywords_elem );

            // generated keyword list? => sort keywords lexicographical (keyword order gives no hint about correct solution)
            if ( my.keywords === true ) entries.sort( ( a, b ) => a.innerHTML.localeCompare( b.innerHTML ) );
          ...
          function evaluate() {
            ...
            $.makeIterable( main_elem.querySelectorAll( '.gap input' ) ).map( ( gap, i ) => {
              ...
              const nearly = gap.value.toLowerCase().trim() === keywords[ i ].word.toLowerCase().trim();
              const correct = gap.value === keywords[ i ].word;
              ...
              gap.disabled = true;
              if ( !nearly ) gap.value = '';
              if ( my.solutions ) gap.setAttribute( 'placeholder', keywords[ i ].word );
