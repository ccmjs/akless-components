...
( function () {
  ...
  const component = {
    ...
    config: {
      ...
      "builder": [ "ccm.component", "https://ccmjs.github.io/akless-components/cloze_builder/versions/ccm.cloze_builder-3.0.0.js", { "submit_button": false } ],
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        function handoverApp( config ) {
          ...
          advance_elem.querySelector( '#embed_code' ).innerHTML = getEmbedCode( self.url, $.getIndex( self.url ), store_settings, config.key );
          ...
          function getEmbedCode( url, index, store_settings, key ) {
            ...
            return $.escapeHTML( '<script src="' + self.url + '"></script><ccm-' + index + ' key=\'["ccm.get",' + JSON.stringify( store_settings ) + ',"' + key + '"]\'></ccm-' + index + '>' );
