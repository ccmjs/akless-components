          ...
          const sections = {
            ...
            results: () => {
              ...
              my.cloze.results.get( '{}', results => {
                ...
                results.map( result => {
                  ...
                  if ( result.key[ 0 ] && !content_elem.querySelector( '#section-results-user-input option[value="' + result.key[ 0 ] + '"]' ) ) $.append( user_elem, $.html( { tag: 'option', value: result.key[ 0 ], inner: result.key[ 0 ] } ) );
                ...
                // render table
                content_elem.querySelectorAll( 'option:only-child' ).length === 2 ? $.setContent( content_elem, my.placeholder.message ) : updateTable();
                ...
                function updateTable() {
                  ...
                  else if ( !user_value &&  key_value ) query = { "_id": { $regex: '(^' + key_value + '$)|(,' + key_value + '(,|$))' } };
            ...
            gaps: () => {
              ...
              my.cloze.results.get( '{}', results => {
                ...
                // render main HTML structure of gaps section
                $.setContent( content_elem, $.html( my.html.gaps, {
                  cloze: my.placeholder.cloze,
                  choose: my.placeholder.choose,
                 ...
                 * @type {Element}
                 */
                const chart_elem = content_elem.querySelector( '#chart' );

                // fill selector boxes with entries
                results.map( result => {
                  if ( Array.isArray( result.key ) ) result.key = result.key[ 1 ];
                  if ( !content_elem.querySelector( '#section-results-key-input  option[value="' + result.key + '"]' ) ) $.append(  key_elem, $.html( { tag: 'option', value: result.key, inner: result.key } ) );
                } );

                // render chart
                updateChart();

                /** (re)renders the gaps chart */
