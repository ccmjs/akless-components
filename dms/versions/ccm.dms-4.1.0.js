        ...
        const view = [
          ...
          async () => {
            ...
            await $.asyncForEach( await apps.get(), async app => {
              add( app_titles, app.title    );
              add( creators  , app.creator  );
              add( categories, app.category );
              if ( !component [ app.path ] ) component [ app.path ] = await components.get( $.getIndex( app.path ) );
              component[ app.path ] && add( component_titles, component[ app.path ].title );
              app.tags     && app.tags    .forEach( tag      => add( tags     , tag      ) );
              app.language && app.language.forEach( language => add( languages, language ) );
            } );
