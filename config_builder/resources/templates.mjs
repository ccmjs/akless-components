  ...
  return html`
    <div class="mb-3" ?data-hidden=${ hidden }>
      ...
      <select class="form-select" name="${ prop }" id="${ id }-${ key }" ?disabled=${ disabled } @change=${ events.onChange }>
        ${ options.map( ( { inner, value } ) => html`<option value="${ value }" ?selected=${ value === initial_value } data-lang="${ key }_${ value }">${ builder.text[ key + '_' + value ] || inner }</option>` ) }
      </select>
    </div>
  `;
}
...
export function buttons( disabled ) {
  return html`
    <footer class="border-top d-flex align-items-center flex-wrap text-nowrap p-3" ?data-hidden=${ !builder.preview && ( !builder.onfinish || !builder.text.submit ) }>
