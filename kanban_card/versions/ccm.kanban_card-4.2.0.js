 ...
 * @version 4.2.0
 * @changes
 * version 4.2.0 (23.04.2021)
 * - realtime update by oninput or onchange via config
...
//    "user": [ "ccm.instance", "https://ccmjs.github.io/akless-components/user/versions/ccm.user-9.7.1.js" ]
    ...
    Instance: function () {
      ...
      const input = ( elem, title_or_deadline ) => {
        ...
        const update = event => this.updateCardProperty( title_or_deadline ? 'title' : 'deadline', event.target.value, true, true );
        if ( this.realtime === 'input' ) input.oninput = update;
        if ( this.realtime === 'change' ) input.onchange = update;
        $.replace( elem, $.html( input ) );
      ...
      const textarea = elem => {
        ...
        const update = event => this.updateCardProperty( 'summary', event.target.value, true, true );
        if ( this.realtime === 'input' ) textarea.oninput = update;
        if ( this.realtime === 'change' ) textarea.onchange = update;
        $.replace( elem, $.html( textarea ) );
