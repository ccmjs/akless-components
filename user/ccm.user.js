    ...
    Instance: function () {
        ...
        do {
          switch ( my.realm ) {
            ...
            case 'hbrsinfpseudo':
              ...
              break;
            default:
              if ( !( result = await renderLogin( this.title, true ) ) ) return this.start();
      ...
      this.logout = async () => {
        ...
        switch ( this.realm ) {
          ...
          case 'hbrsinfkaul':
          case 'hbrsinfpseudo':
            await this.ccm.load( { url: 'https://kaul.inf.h-brs.de/login/logout.php', method: 'JSONP', params: { realm: this.realm } } );
          break;
          default:
            await this.ccm.load( { url: self.url, method: 'POST', params: { realm: this.realm, token: this.token } } );
            break;
        }

        // clear user data
        this.user = undefined; this.token = undefined;
